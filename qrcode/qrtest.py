...
def read_qrcode(image_path, roi_cnt=1):
    ...
    qrcodes = []
    for roi in rois:
       qrcodes.append(pyzbar.decode(roi, symbols=[pyzbar.ZBarSymbol.QRCODE]))
    # 处理解码结果
...
for filename in os.listdir(folder_path):
    if filename.endswith((".bmp", "png")):  # 只处理 .bmp 文件
        ...
        if qrcodes:
            print(f"pyzbar roi{roi_cnt} {filename} ", qrcodes)
        else:
            print(f"pyzbar roi{roi_cnt} {filename} None\r", end='', flush=True)