...
def depth_2_color_space(kinect, depth_space_point, depth_frame_data, show=False, return_aligned_image=False):
    ...
    color2depth_points_type = depth_space_point * np.int32(1920 * 1080)
    ...
    depthXYs += 0.5
    depthXYs = depthXYs.reshape(kinect.color_frame_desc.Height, kinect.color_frame_desc.Width, 2).astype(np.int32)
...
def color_2_depth_space(kinect, color_space_point, depth_frame_data, show=False, return_aligned_image=False):
    ...
    depth2color_points_type = color_space_point * np.int32(512 * 424)
    ...
    colorXYs += 0.5
    colorXYs = colorXYs.reshape(kinect.depth_frame_desc.Height, kinect.depth_frame_desc.Width, 2).astype(np.int32)
...
def depth_2_world(kinect, depth_frame_data, camera_space_point, as_array=False):
    ...
    import numpy as np
    import ctypes
    depth2world_points_type = camera_space_point * np.int32(512 * 424)
...
def color_2_world(kinect, depth_frame_data, camera_space_point, as_array=False):
    ...
    import numpy as np
    import ctypes
    color2world_points_type = camera_space_point * np.int32(1920 * 1080)
...
def world_point_2_color(kinect, camera_space_point, point):
    ...
    import ctypes
    import numpy as np
    world_point_data_type = camera_space_point * np.int32(1)
...
def depth_points_2_world_points(kinect, depth_space_point, depth_points):
    ...
    import ctypes
    import numpy as np
    depth2world_point_type = depth_space_point * np.int32(1)
...
def depth_points_2_camera_points(kinect, depth_space_point, camera_space_point, xys, as_array=False):
    ...
    depth_points_type = depth_space_point * np.int32(length_of_points)
    depth_points = ctypes.cast(depth_points_type(), ctypes.POINTER(depth_space_point))
    camera_points_type = camera_space_point * np.int32(length_of_points)
    camera_points = ctypes.cast(camera_points_type(), ctypes.POINTER(camera_space_point))
    depths = ctypes.POINTER(ctypes.c_ushort) * np.int32(length_of_points)
...
def depth_point_2_world_point(kinect, depth_space_point, depthPoint):
    ...
    import numpy as np
    import ctypes
    depth_point_data_type = depth_space_point * np.int32(1)
...
def depth_point_2_color(kinect, depth_space_point, depthPoint):
    ...
    import numpy as np
    import ctypes
    depth_point_type = depth_space_point * np.int32(1)
...
def depth_2_world_table(kinect, depth_space_point, as_array=False):
    ...
    import numpy as np
    import ctypes
    table = depth_space_point * np.int32(512 * 424)
