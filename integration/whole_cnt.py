...
def depth2level(src_list, thresh=10):
...
def get_roi_means(img, data, layer):
    h, w = img.shape[:2]
    roi_means = []
    for line in data:
        if line[-1] != layer:
...
def front_proc(input, front_view):
    m = len(input)
    n = len(input[0])
    for i in range(n):
        b = front_view[i]
...
def right_proc(input, right_view):
    m = len(input)
    n = len(input[0])
    for i in range(m):
        b = right_view[i]
...
label_folder = 'my_test_output/detection_results2/labels'
depth_folder = 'my_test_output/detection_results2/depth'
    ...
    for cur_layer in range(max_layer+1):
        ...
        if width != 1 and length != 1:
            ...
            for i in range(4):
                ...
                layer_roi_means = get_roi_means(depth_img, bat_label_data[i], cur_layer)
                layer_level = depth2level(layer_roi_means)
                view_levels.append(layer_level)
            flag_mat = front_proc(flag_mat, view_levels[0])
            flag_mat = right_proc(flag_mat, view_levels[1])
            flag_mat = back_proc(flag_mat, view_levels[2])
            flag_mat = left_proc(flag_mat, view_levels[3])
